...
if (!$conn) {
    ...
    $backup = array();
    $date = "";
    $s = oci_parse($conn, "SELECT TO_CHAR(backup_date, 'yyyy-mm-dd') backup_date, available FROM backups WHERE backup_type='recreateDb' AND server_id=" . $_GET["id"] . " ORDER BY backup_id DESC");
